...
(function () {
  ...
  function deteccionFacial(detectarPuntos) {
    ...
    if (detectarPuntos) {
      ...
      for (var i = 0; i < positions.length; i++) {
        ...
        ellipse(positions[i][0], positions[i][1], 20, 20);
      }
    }
    var cara = {
      ojoDerecho: {
        x: positions[27][0],
        y: positions[27][1],
        d: dist(positions[17][0], positions[17][0], positions[32][0], positions[32][0])
      },
      ojoIzquierdo: {
        x: positions[32][0],
        y: positions[32][1]
  ...
  function vistaCabina(cara) {
    ...
    if (!cara) {
      rect(0, 0, cnv.width, mascara.height);
      return;
    }

    var x = cara.centro.x;
    var y = cara.centro.y;
    if (y + mascara.height < cnv.height / 2) {
      rect(0, 0, cnv.width, mascara.height);
      cara = undefined;
      return;
    }
      ...
      controller.loop(function (frame) {
        ...
        if (frame.gestures.length) {
          ...
          if (circulos.length && frame.hands.length == 1 && frame.hands[0].type == 'right') {
            ...
            habilitarHud = Leap.vec3.dot(dir, circulos[0].normal) > 0;

            ultimaAccion = (habilitarHud ? 'A' : 'DESA') + 'CTIVAR HUD';
            setTimeout(function () {
              ultimaAccion = ''
            }, 3000);
          }
